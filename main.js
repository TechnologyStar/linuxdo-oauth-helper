...
(function() {
    ...
    class Utils {
        ...
        static generateActionStats(history) {
            ...
            history.forEach(record => {
                if (record.action === '自动授权') {
                    actions['自动授权']++;
                } else if (record.action === '手动授权') {
                    actions['手动授权']++;
    ...
    class AutoClickManager {
        constructor(storage, settings) {
            ...
            this.isEnabled = settings.autoClickApprove;
            this.pageInfo = new PageInfo();

            Utils.log(`自动点击管理器 - 状态: ${this.isEnabled ? '启用' : '禁用'}`);
        ...
        attemptAutoClick() {
            ...
            if (!this.hasClicked) {
                this.storage.updateStats('autoClicks');
                this.storage.updateStats('totalLogins');
                Utils.log('自动点击统计已更新');
            }

            if (this.settings.showNotifications) {
                this.showNotification('即将自动授权...');
            }

            this.hasClicked = true;

            setTimeout(() => {
                Utils.log(`跳转到: ${approveButton.href}`);
            ...
            ui.innerHTML = `
                ...
                <div class="oauth-panel" id="oauthPanel">
                    <div class="oauth-header">
                        ...
                        <div class="oauth-version">v${CONFIG.version} - 面板集成版</div>
                    </div>
        ...
        generateStatsHTML() {
            ...
            return `
                <div class="oauth-section">
                    ...
                    <div class="oauth-stat-grid">
                        <div class="oauth-stat-card">
                            <div class="oauth-stat-value">${stats.autoClicks || 0}</div>
                            <div class="oauth-stat-label">自动点击</div>
                        </div>
                        <div class="oauth-stat-card">
            ...
            const helpContent = `
                <div style="max-width: 500px; line-height: 1.6;">
                    ...
                    <div style="margin-bottom: 16px;">
                        ...
                        <p>• <strong>自动点击授权</strong>：自动处理OAuth授权流程</p>
            ...
            const aboutContent = `
                <div style="max-width: 450px; text-align: center; line-height: 1.6;">
                    <div style="margin-bottom: 20px;">
                        ...
                        <p style="color: ${CONFIG.uiThemes[this.currentUITheme].colors.textMuted}; margin: 8px 0;">专为LINUX DO OAuth设计的极简助手</p>
                    </div>
